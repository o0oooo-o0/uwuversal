...
getgenv().__PhCF = nil
if fontAssetId then
    getgenv().__PhCF = Font.new(fontAssetId)
end
...
local Library = {
    ...
    NotifyConfig = {
        ...
        BarSide   = 'Left';   -- 'Left' | 'Right' | 'Top' | 'Bottom'
        PositionX = 0;
        PositionY = 40;
        Animation = 'Default'; -- 'Default'|'Fade'|'Bounce'|'Elastic'|'Slide'|'Zoom'|'Flip'|'Spiral'|'Pulse'|'NoAnim'
    };

    -- Transparencies (set via sliders in groupboxes)
    MenuBgTransparency      = 0;
...
-- Default stub so Library:Toggle() can be called safely before
-- CreateWindow() is invoked (e.g. from mobile UI buttons).
-- CreateWindow() replaces this with the real implementation.
function Library:Toggle()
    -- No-op until CreateWindow() installs the real Toggle.
end;
...
function Library:Create(Class, Properties)
    ...
    if getgenv().__PhCF and (_Instance:IsA("TextLabel") or _Instance:IsA("TextBox") or _Instance:IsA("TextButton")) then
        _Instance.FontFace = getgenv().__PhCF
    end
...
function Library:ApplyTopGlow(Parent, zindex)
...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        local ColorPicker = {
            ...
            Type = 'ColorPicker';
            Title = (function() return type(Info.Title) == 'string' and Info.Title or 'Color picker' end)(),
        ...
        local PickerFrameOuter = Library:Create('Frame', {
            ...
            Size = UDim2.fromOffset(230, (function() if Info.Transparency then return 271 else return 253 end end)());
            Visible = false;
            ZIndex = 15;
            Parent = ScreenGui,
        });
        ...
        do
            ...
            if parentObj.ToggleRegion and parentObj._ColorPickerCount ~= nil then
                parentObj._ColorPickerCount = parentObj._ColorPickerCount + 1

                local baseWidth = 170
                local shrinkPerPicker = 32
                local newWidth = baseWidth - math.max(0, parentObj._ColorPickerCount - 1) * shrinkPerPicker
                parentObj.ToggleRegion.Size = UDim2.new(0, newWidth, 1, 0)
            end
        end
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        function KeyPicker:Update()
            ...
            ContainerLabel.TextColor3 = (function() return State and Library.AccentColor or Library.FontColor end)();
            Library.RegistryMap[ContainerLabel].Properties.TextColor3 = (function() return State and 'AccentColor' or 'FontColor' end)();
...
do
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        Library:ApplyTopGlow(TextBoxInner, 7);
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        Library:ApplyTopGlow(ToggleInner, 7);
        ...
        local function _UpdateToggleLabelWidth()
            local containerWidth = Container.AbsoluteSize.X
            if containerWidth > 20 then
                ToggleLabel.Size = UDim2.new(0, containerWidth - 19, 1, 0)
            end
        end
        Container:GetPropertyChangedSignal('AbsoluteSize'):Connect(_UpdateToggleLabelWidth)
        task.spawn(_UpdateToggleLabelWidth)
        ...
        function Toggle:Display()
            ToggleInner.BackgroundColor3 = (function() return Toggle.Value and Library.AccentColor or Library.MainColor end)();
            ToggleInner.BorderColor3 = (function() return Toggle.Value and Library.AccentColorDark or Library.OutlineColor end)();

            Library.RegistryMap[ToggleInner].Properties.BackgroundColor3 = (function() return Toggle.Value and 'AccentColor' or 'MainColor' end)();
            Library.RegistryMap[ToggleInner].Properties.BorderColor3 = (function() return Toggle.Value and 'AccentColorDark' or 'OutlineColor' end)();
        end;
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        local SliderGradient = Library:Create('Frame', {
            BackgroundColor3 = Color3.new(1, 1, 1);
            BorderSizePixel = 0;
            Size = UDim2.new(1, 0, 1, 0);
            ZIndex = 8;
        ...
        local function GetSliderMaxSize()
            ...
            return (w > 4) and (w - 2) or Slider.MaxSize
        end
        ...
        function Slider:Display()
            ...
            if Info.Compact then
                ...
                local pct = math.floor(
                    ((Slider.Value - Slider.Min) / (Slider.Max - Slider.Min)) * 100 + 0.5
                )
                DisplayLabel.Text = Info.Text .. ':  ' .. pct .. '%'
            end

            local dynMax = GetSliderMaxSize()
            local X = math.ceil(Library:MapValue(Slider.Value, Slider.Min, Slider.Max, 0, dynMax));
            Fill.Size = UDim2.new(0, X, 1, 0);

            HideBorderRight.Visible = not (X == dynMax or X == 0);
        end;
        ...
        SliderInner.InputBegan:Connect(function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and not Library:MouseIsOverOpenedFrame() then
                ...
                local function UpdateSlider(PosX)
                    ...
                    local nX = math.clamp(Diff, 0, GetSliderMaxSize())
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        Library:ApplyTopGlow(DropdownInner, 7);
        ...
        function Dropdown:Display()
            ...
            ItemList.Text = (function() return (Str == '' and '--' or Str) end)();
        end;
        ...
        function Dropdown:BuildDropdownList()
            ...
            for Idx, Value in next, Values do
                ...
                function Table:UpdateButton()
                    ...
                    ButtonLabel.TextColor3 = (function() return Selected and Library.AccentColor or Library.FontColor end)();
                    Library.RegistryMap[ButtonLabel].Properties.TextColor3 = (function() return Selected and 'AccentColor' or 'FontColor' end)();
                end;
    ...
    -- ── AddTransparencySliders ───────────────────────────────────────────────
    -- Adds two sliders to this groupbox:  Menu Background + Keybind Transparency
    -- Usage: groupbox:AddTransparencySliders()
    function Funcs:AddTransparencySliders()
        self:AddSlider('__MenuBgAlpha', {
            ...
            Suffix  = '%';
            Callback = function(v)
                Library.MenuBgTransparency = v / 100
                for _, entry in pairs(Library.Registry) do
                    if entry.Object and entry.Properties then
                        local prop = entry.Properties.BackgroundColor3
                        if prop == 'MainColor' or prop == 'BackgroundColor' then
                            pcall(function()
                                entry.Object.BackgroundTransparency = Library.MenuBgTransparency
                            end)
                        end
                    end
                end
        ...
        self:AddSlider('__KeybindAlpha', {
            ...
            Suffix  = '%';
            Callback = function(v)
                Library.KeybindTransparency = v / 100
                if Library.KeybindFrame then
                    for _, d in ipairs(Library.KeybindFrame:GetDescendants()) do
                        if d:IsA('Frame') then
                            pcall(function() d.BackgroundTransparency = Library.KeybindTransparency end)
                        end
                    end
                end
            end;
...
function Library:Notify(Text, Time)
    ...
    local _isVertBar = (barSide == 'Left' or barSide == 'Right')
    ...
    -- helper: fade all notification parts in/out
    local function FadeNotif(transparency, duration)
        local info = TweenInfo.new(duration or 0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        for _, obj in ipairs({NotifyInner, InnerFrame, NotifyTopGlow, AccentBar, AccentBarL, AccentBarR}) do
            TweenService:Create(obj, info, {BackgroundTransparency = transparency}):Play()
        end
        TweenService:Create(NotifyLabel, info, {TextTransparency = transparency}):Play()
    end
    ...
    task.spawn(function()
        ...
        elseif anim == 'Fade' then
            ...
            NotifyTopGlow.BackgroundTransparency = 1
            AccentBar.BackgroundTransparency = 1
            AccentBarL.BackgroundTransparency = 1
            AccentBarR.BackgroundTransparency = 1
        ...
        elseif anim == 'Bounce' then
            ...
            NotifyOuter.Size = UDim2.new(0, finalWidth, 0, YSize)
            NotifyOuter.Position = UDim2.new(
                NotifyOuter.Position.X.Scale, offX,
                NotifyOuter.Position.Y.Scale, NotifyOuter.Position.Y.Offset)
            TweenService:Create(NotifyOuter, TweenInfo.new(0.5, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out),
                {Position = UDim2.new(NotifyOuter.Position.X.Scale, 0, NotifyOuter.Position.Y.Scale, NotifyOuter.Position.Y.Offset)}):Play()
        ...
        elseif anim == 'Elastic' then
            ...
            NotifyOuter.Position = UDim2.new(
                NotifyOuter.Position.X.Scale, offX,
                NotifyOuter.Position.Y.Scale, NotifyOuter.Position.Y.Offset)
            TweenService:Create(NotifyOuter, TweenInfo.new(0.6, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out),
                {Position = UDim2.new(NotifyOuter.Position.X.Scale, 0, NotifyOuter.Position.Y.Scale, NotifyOuter.Position.Y.Offset)}):Play()

        elseif anim == 'Slide' then
            NotifyOuter.Size = UDim2.new(0, finalWidth, 0, YSize)
            local origY = NotifyOuter.Position.Y.Offset
            NotifyOuter.Position = UDim2.new(NotifyOuter.Position.X.Scale, NotifyOuter.Position.X.Offset, 0, -YSize - 10)
            TweenService:Create(NotifyOuter, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                {Position = UDim2.new(NotifyOuter.Position.X.Scale, NotifyOuter.Position.X.Offset, 0, origY)}):Play()

        elseif anim == 'Zoom' then
            NotifyOuter.Size = UDim2.new(0, 4, 0, 4)
            NotifyInner.BackgroundTransparency = 1
            InnerFrame.BackgroundTransparency = 1
            NotifyTopGlow.BackgroundTransparency = 1
            AccentBar.BackgroundTransparency = 1
        ...
        elseif anim == 'Pulse' then
            NotifyOuter.Size = UDim2.new(0, finalWidth * 1.3, 0, YSize * 1.3)
            NotifyInner.BackgroundTransparency = 1
            InnerFrame.BackgroundTransparency = 1
            NotifyTopGlow.BackgroundTransparency = 1
            AccentBar.BackgroundTransparency = 1
        ...
        else -- Default: original swipe expand
    ...
    task.spawn(function()
        ...
        elseif anim == 'Bounce' or anim == 'Elastic' then
            ...
            TweenService:Create(NotifyOuter, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
                {Position = UDim2.new(NotifyOuter.Position.X.Scale, offX, NotifyOuter.Position.Y.Scale, NotifyOuter.Position.Y.Offset)}):Play()
            wait(0.35)
            NotifyOuter:Destroy()

        elseif anim == 'Slide' then
            TweenService:Create(NotifyOuter, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
                {Position = UDim2.new(NotifyOuter.Position.X.Scale, NotifyOuter.Position.X.Offset, 0, -YSize - 20)}):Play()
            wait(0.35)
            NotifyOuter:Destroy()

        elseif anim == 'Zoom' then
            FadeNotif(1, 0.2)
            TweenService:Create(NotifyOuter, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.In),
                {Size = UDim2.new(0, 4, 0, 4)}):Play()
            wait(0.3)
            NotifyOuter:Destroy()
...
function Library:CreateWindow(...)
    ...
    -- ── Live animated title ─────────────────────────────────────────────────
    -- anim: 'None'|'Slide'|'Fade'|'TypeWriter'|'Bounce'|'Flash'
    function Window:SetTitle(text, animType)
        ...
        elseif animType == 'Fade' then
            TweenService:Create(lbl, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {TextTransparency=1}):Play()
        ...
        elseif animType == 'Bounce' then
            ...
            TweenService:Create(lbl, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                {Position = origPos + UDim2.fromOffset(0, -5)}):Play()
            task.delay(0.11, function()
                lbl.Text = text
                TweenService:Create(lbl, TweenInfo.new(0.25, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out),
                    {Position = origPos}):Play()
            end)
        else -- Slide
            local origPos = lbl.Position
            TweenService:Create(lbl, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
                {Position = origPos + UDim2.fromOffset(10, 0), TextTransparency=1}):Play()
            task.delay(0.13, function()
                lbl.Text = text
                lbl.Position = origPos - UDim2.fromOffset(10, 0)
                TweenService:Create(lbl, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                    {Position = origPos, TextTransparency=0}):Play()
            end)
        end
    end
    ...
    -- ── Tab button size override ─────────────────────────────────────────────
    -- x = width px (nil = keep auto), y = height px (nil = keep auto)
    function Window:SetTabSize(x, y)
        for _, btn in ipairs(TabArea:GetChildren()) do
            if btn:IsA('Frame') then
                local cx = x and UDim2.new(0, x, btn.Size.Y.Scale, btn.Size.Y.Offset)
                              or btn.Size
    ...
    -- ── UI Glow ─────────────────────────────────────────────────────────────
    -- Returns a controller {SetEnabled, SetColor, SetAmount}
    function Window:SetGlow(enabled, color, amount)
        Library.GlowEnabled = enabled ~= nil and enabled or Library.GlowEnabled
        Library.GlowColor   = color  or Library.GlowColor
        Library.GlowAmount  = amount or Library.GlowAmount

        -- remove old glow
        local existing = Outer:FindFirstChild('__UIGlow')
        ...
        gl.ImageColor3 = Library.GlowColor
        gl.ImageTransparency = 1 - (Library.GlowAmount / 20)
        gl.ScaleType = Enum.ScaleType.Slice
        gl.SliceCenter = Rect.new(50,50,450,450)
        gl.AnchorPoint = Vector2.new(0.5, 0.5)
        gl.Position = UDim2.new(0.5, 0, 0.5, 0)
        gl.Size = UDim2.new(1, Library.GlowAmount * 5, 1, Library.GlowAmount * 5)
        gl.ZIndex = 0
        gl.Parent = Outer
    end
    function Window:AddTab(Name)
        ...
        Library:AddToRegistry(TabIndicatorGlow, { BackgroundColor3 = 'AccentColor' });
        ...
        function Tab:AddGroupbox(Info)
            ...
            local BoxOuter = Library:Create('Frame', {
                ...
                ZIndex = 2;
                Parent = (function() return Info.Side == 1 and LeftSide or RightSide end)();
            });
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(Name)
                ...
                local TabHLLeft = Library:Create('Frame', {
                    BackgroundColor3 = Library.AccentColor;
                    BorderSizePixel = 0;
                    AnchorPoint = Vector2.new(1, 0);
                    Position = UDim2.new(0.5, 0, 0, 0);
                ...
                local TabHLRight = Library:Create('Frame', {
                    BackgroundColor3 = Library.AccentColor;
                    BorderSizePixel = 0;
                    AnchorPoint = Vector2.new(0, 0);
                    Position = UDim2.new(0.5, 0, 0, 0);
                ...
                Library:AddToRegistry(TabHLLeft, {
                    BackgroundColor3 = 'AccentColor';
                });
                Library:AddToRegistry(TabHLRight, {
                    BackgroundColor3 = 'AccentColor';
                });
                local ButtonLabel = Library:CreateLabel({
                    Size = UDim2.new(1, 0, 1, 0);
...
Library.MobileKeybindPositions = {}
Library.MobileKeybinds = {}

local function _mkb_savePositions()
    local ok, err = pcall(function()
        if not writefile then return end
        local data = {}
        for id, btn in pairs(Library.MobileKeybinds) do
            local pos = btn.Outer.Position
            data[id] = { X = pos.X.Offset, Y = pos.Y.Offset }
        end
        writefile("linoria_mkb_positions.json", game:GetService("HttpService"):JSONEncode(data))
    end)
end

local function _mkb_loadPositions()
    local ok, result = pcall(function()
        if not readfile or not isfile then return {} end
        if not isfile("linoria_mkb_positions.json") then return {} end
        return game:GetService("HttpService"):JSONDecode(readfile("linoria_mkb_positions.json"))
    end)
    return (ok and result) or {}
end

local _mkb_savedPos = _mkb_loadPositions()

function Library:CreateMobileKeybind(id, info)
    assert(type(id) == 'string', 'CreateMobileKeybind: id must be a string')
    info = info or {}

    local text     = info.Text     or id
    local defState = info.Default  or false
    local callback = info.Callback or function() end

    local guiParent
    local existingMobileGui = CoreGui:FindFirstChild("LinoriaMobileUI")
    if existingMobileGui then
        guiParent = existingMobileGui
    else
        local mg = Instance.new("ScreenGui")
        mg.Name = "LinoriaMobileUI"
        mg.ZIndexBehavior = Enum.ZIndexBehavior.Global
        ProtectGui(mg)
        mg.Parent = CoreGui
        guiParent = mg
    end

    local savedPos = _mkb_savedPos[id]
    local initPos
    if savedPos then
        initPos = UDim2.new(0, savedPos.X, 0, savedPos.Y)
    else
        local count = 0
        for _ in pairs(Library.MobileKeybinds) do count = count + 1 end
        initPos = UDim2.new(1, -100, 0, 10 + count * 44)
    end
    ...
    Library.MobileKeybinds[id] = MKB

    local function updateDisplay()
        Btn.TextColor3 = MKB.State
            and Library.AccentColor
            or  Color3.fromRGB(255, 255, 255)
    end
    updateDisplay()

    function MKB:GetState()
        return MKB.State
    end

    function MKB:SetState(bool)
        MKB.State = not not bool
        updateDisplay()
        Library:SafeCallback(callback, MKB.State)
        if MKB.Changed then
            Library:SafeCallback(MKB.Changed, MKB.State)
        end
    end

    function MKB:OnChanged(fn)
        MKB.Changed = fn
        fn(MKB.State)
    end

    function MKB:Destroy()
        Library.MobileKeybinds[MKB.Id] = nil
        Outer:Destroy()
    end
        ...
        or input.UserInputType == Enum.UserInputType.Touch then
            ...
            conn = input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    ...
                    conn:Disconnect()
                    if not hasMoved then
                        MKB:SetState(not MKB.State)
                    else
                        _mkb_savedPos[id] = {
                            X = Outer.Position.X.Offset,
                            Y = Outer.Position.Y.Offset,
                        }
                        _mkb_savePositions()
                    end
                end
    ...
    local _origUpdateColors = Library.UpdateColorsUsingRegistry
    Library.UpdateColorsUsingRegistry = function(self)
        _origUpdateColors(self)

        for _, mkb in pairs(Library.MobileKeybinds) do
            if mkb.State then
                mkb.Btn.TextColor3 = Library.AccentColor
            end
        end
    end
